...
URL模板由\@{name}\@表示内部变量替换，其中的变量名则为JSON中可以访问的内部变量名，内部变量的值是前端在解析JSON时进行替换，而在传输时不做替换；

而诸如\@{$name}\@的形式表示外部变量替换，仅仅用于API说明，使用时应该用具体的外部变量替换后传输。
\code+[json]{begin}
{
    "posts": {
        "id": "152",
...
\list*{
	...
	\* \@source\@（必需） 该请求的源资源所在的URL表示，当\@source\@与\@self\@相同时，表示直接访源资源；否则，则访问的是一个引用资源或者对源资源进行了部分访问（比如通过滤器）。
