...
如图所示，用户可以通过多种客户端访问。客户端向服务器请求两类数据：一类是Web数据，包括网页模板、样式表、网页程序的代码（Javascript）、图片等多媒体数据，无需授权；另外一类数据是关于论坛的用户产生内容，多数需要授权，用API的方式访问（JSON格式），在客户端中完成对用户界面的数据填充。后端的Web服务器负责提供固定入口和静态内容，而API服务器负责提供动态数据。
...
API交换的数据结构是面向前端的，主要资源根据缓存级别分成动态性较弱的资源（e.g. /resources/posts）和动态性较强的资源（e.g. /resources/stats/posts），兼顾后端的数据库内数据结构而设计。API数据结构并不等价于数据库数据结构。数据库表的设计主要考虑的是合理的抽象对象间关系，合理的读写分离，合理的区别原始数据（需要持久化，无法重建）和计算后数据（没有持久化必要，可以随时重建）。
