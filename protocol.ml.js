...
前后端分离的本质是数据和呈现的分离，后端关注数据储存，前端关注数据呈现，这样做的好处有很多。首先，前端关注的是人机交互，是数据的直接呈现者，如果向以往那样由后端提供渲染完成的页面，则会对前端开发带来很大的麻烦。前端往往需要关注多个平台，而且常常有布局的调整，内容的删减、添加，如果这样的需求统统都需要后端来配合，则代价太大。而且，往往后端会把页面呈现和事物逻辑混杂在一起，导致后端代码难以维护。其次，前端通过API读取数据，渲染页面，可以分摊后端的压力，利用用户浏览器的计算能力去呈现代码，这对于论坛这类动态内容为主的网站特别合适（静态网站则没有这个优势，因为后端只需要要渲染一次），每个用户渲染的都是不同的页面。再次，用户个性化自己的页面，其他客户端（如手机、平板等）开发需要设计更加专注于客户端的页面，而前后端分离的设计，为这些需求提供了充足的空间。

缓存优化也是API设计的目标，它的本质是让资源合理的分割，不再仅仅按照逻辑关系分割数据，而是按照数据的逻辑关系+生存期和更新频率分割，分割后的数据则可以采用不同的缓存策略去管理。过去一个页面中包含了各种生存期、各种更新频率的内容，由于都在后端渲染成页面，数据的缓存仅仅依赖数据库和系统的file cache，当同时在线人数上升时，后端前台网页服务器和后台数据库均不堪重负。而API设计时考虑了把适合不同缓存策略的数据分开，充分利用HTTP提供的客户端缓存，同时也允许后端实现中间表示层的缓存。想必能大大改善系统性能，服务更多的用户。
